...
class TestConstraints(unittest.TestCase):
    ...
    def test_constr_REG_GEN_EX(self):
        ...
        for case in test_cases.CASES:
            ...
            # Constraint
            constr = pf.Constraint('extended voltage regulation by generators',net)
            self.assertEqual(constr.name,'extended voltage regulation by generators')
            ...
            rowsJ = (net.get_num_reg_gens()-net.get_num_slack_gens())
                        ...
                        if not flags[(t,bus.index)]:
                            if bus.is_regulated_by_gen() and not bus.is_slack():
                                for gen in bus.reg_generators:
                                    ...
                                    a = (Qmax-Q)*(Q-Qmin)
                                    b = (v-vs)**2.
                                    Comp = a+b-np.sqrt(a**2.+b**2.+2*eps)
                                    self.assertAlmostEqual(Comp,f[J_row])
                                    J_row += 1
                        flags[(t,bus.index)] = True            
