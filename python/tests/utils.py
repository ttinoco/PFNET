...
def compare_two_networks(unittest, net, new_net):
    ...
    Raises an exception if all network elements do not match or objects are not distinct
    """
    ...
    for i in range(net.num_buses):
        ...
        unittest.assertEqual(bus.is_regulated_by_gen(),
                         new_bus.is_regulated_by_gen())
        unittest.assertEqual(bus.is_regulated_by_tran(),
                         new_bus.is_regulated_by_tran())
        unittest.assertEqual(bus.is_regulated_by_shunt(),
                         new_bus.is_regulated_by_shunt())
    ...
    for i in range(net.num_branches):
        ...
        unittest.assertEqual(branch.is_phase_shifter(),
                         new_branch.is_phase_shifter())
        unittest.assertEqual(branch.is_tap_changer(), new_branch.is_tap_changer())
        unittest.assertEqual(branch.is_tap_changer_v(),
                         new_branch.is_tap_changer_v())
        unittest.assertEqual(branch.is_tap_changer_Q(),
                         new_branch.is_tap_changer_Q())
        if branch.is_tap_changer_v():
            unittest.assertEqual(branch.reg_bus.index, new_branch.reg_bus.index)
        unittest.assertLess(norminf(branch.g - new_branch.g),
                        eps * (1 + norminf(branch.g)))
        unittest.assertLess(norminf(branch.g_k - new_branch.g_k),
                        eps * (1 + norminf(branch.g_k)))
        unittest.assertLess(norminf(branch.g_m - new_branch.g_m),
                        eps * (1 + norminf(branch.g_m)))
        unittest.assertLess(norminf(branch.b - new_branch.b),
                        eps * (1 + norminf(branch.b)))
        unittest.assertLess(norminf(branch.b_k - new_branch.b_k),
                        eps * (1 + norminf(branch.b_k)))
        unittest.assertLess(norminf(branch.b_m - new_branch.b_m),
                        eps * (1 + norminf(branch.b_m)))
        ...
        unittest.assertEqual(branch.has_pos_ratio_v_sens(),
                         new_branch.has_pos_ratio_v_sens())
    ...
    for i in range(net.num_generators):
        ...
        unittest.assertEqual(gen.is_P_adjustable(),
                         new_gen.is_P_adjustable())
        if gen.is_regulator():
        ...
        unittest.assertLess(norminf(gen.cost_coeff_Q0 -
                                new_gen.cost_coeff_Q0), eps)
        unittest.assertLess(norminf(gen.cost_coeff_Q1 -
                                new_gen.cost_coeff_Q1), eps)
        unittest.assertLess(norminf(gen.cost_coeff_Q2 -
                                new_gen.cost_coeff_Q2), eps)

    # Var generators
    unittest.assertEqual(net.num_var_generators,
                     new_net.num_var_generators)
    ...
    for i in range(net.num_shunts):
        ...
        unittest.assertEqual(shunt.is_switched_v(),
                         new_shunt.is_switched_v())
        if shunt.is_switched_v():
            unittest.assertEqual(shunt.reg_bus.index,
                             new_shunt.reg_bus.index)
        unittest.assertLess(norminf(shunt.g - new_shunt.g),
                        eps * (1 + norminf(shunt.g)))
        # unittest.assertLess(norminf(shunt.b-new_shunt.b),eps*(1+norminf(shunt.b)))
        unittest.assertLess(norminf(shunt.b_max - new_shunt.b_max),
                        eps * (1 + norminf(shunt.b_max)))
        unittest.assertLess(norminf(shunt.b_min - new_shunt.b_min),
                        eps * (1 + norminf(shunt.b_min)))
    ...
    for i in range(net.num_loads):
        ...
        unittest.assertLess(
            norminf(load.target_power_factor - new_load.target_power_factor), eps)
        unittest.assertLess(norminf(load.util_coeff_Q0 -
                                new_load.util_coeff_Q0), eps)
        unittest.assertLess(norminf(load.util_coeff_Q1 -
                                new_load.util_coeff_Q1), eps)
        unittest.assertLess(norminf(load.util_coeff_Q2 -
                                new_load.util_coeff_Q2), eps)
