/** @file constr_FACTS_QSET_SWITCH.c
 *  @brief This file defines the data structure and routines associated with the constraint of type FACTS_QSET_SWITCH.
 *
...
#include <pfnet/constr_FACTS_QSET_SWITCH.h>

Constr* CONSTR_FACTS_QSET_SWITCH_new(Net* net) {
  Constr* c = CONSTR_new(net);
  CONSTR_set_func_count_step(c,&CONSTR_FACTS_QSET_SWITCH_count_step);
  CONSTR_set_func_analyze_step(c,&CONSTR_FACTS_QSET_SWITCH_analyze_step);
  CONSTR_set_func_eval_step(c,&CONSTR_FACTS_QSET_SWITCH_eval_step);
  CONSTR_set_func_store_sens_step(c,&CONSTR_FACTS_QSET_SWITCH_store_sens_step);
  CONSTR_set_name(c,"switching FACTS reactive power control");
  return c;
}
...
void CONSTR_FACTS_QSET_SWITCH_eval_step(Constr* c, Bus* bus, BusDC* busdc, int t, Vec* values, Vec* values_extra) {
  // Nothing to do
}
