...
void CONSTR_PF_eval_step(Constr* c, Branch* br, int t, Vec* var_values) {
  ...
  for (k = 0; k < 2; k++) {
    ...
    if (var_w[k]) { // wk var
      ...
      J[data->dPdw_indices[bus_index_t[k]]] += flowQ[k];  // dPk/dwk
      J[data->dQdw_indices[bus_index_t[k]]] -= flowP[k]; // dQk/dwk
      
      // H
      Hcounter_val = Hcounter[bus_index_t[k]];
      HP[k][data->dwdw_indices[bus_index_t[k]]] += flowP[k]; // wk and wk
      HQ[k][data->dwdw_indices[bus_index_t[k]]] += flowQ[k];
      if (var_v[k]) { // wk and vk
	HP[k][data->dwdv_indices[bus_index_t[k]]] += flowQ[k]/v[k]; // wk and wk
	HQ[k][data->dwdv_indices[bus_index_t[k]]] -= flowP[k]/v[k];
      }
      if (var_w[m]) { // wk and wm
    ...
    if (var_v[k]) { // vk var
      ...
      J[data->dPdv_indices[bus_index_t[k]]] -= 2*flowP_sh[k]/v[k] + flowP[k]/v[k]; // dPk/dvk
      J[data->dQdv_indices[bus_index_t[k]]] -= 2*flowQ_sh[k]/v[k] + flowQ[k]/v[k]; // dQk/dvk

      // H
      Hcounter_val = Hcounter[bus_index_t[k]];
      HP[k][data->dvdv_indices[bus_index_t[k]]] -= 2.*flowP_sh[k]/(v[k]*v[k]); // vk and vk
      HQ[k][data->dvdv_indices[bus_index_t[k]]] -= 2.*flowQ_sh[k]/(v[k]*v[k]);
      if (var_w[m]) { // vk and wm
