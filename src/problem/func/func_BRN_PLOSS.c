/** @file func_BRN_PLOSS.c
 *  @brief This file defines the data structure and routines associated with the function of type BRN_PLOSS.
 *
...
#include <pfnet/func_BRN_PLOSS.h>

Func* FUNC_BRN_PLOSS_new(REAL weight, Net* net) {
    Func* f = FUNC_new(weight, net);
    FUNC_set_func_count_step(f, &FUNC_BRN_PLOSS_count_step);
    FUNC_set_func_analyze_step(f, &FUNC_BRN_PLOSS_analyze_step);
    FUNC_set_func_eval_step(f, &FUNC_BRN_PLOSS_eval_step);
    FUNC_set_name(f, "branch P loss");
    return f;
}
...
void FUNC_BRN_PLOSS_eval_step(Func* f, Bus* bus, BusDC* busdc, int t, Vec* var_values) {
    ...
    for (br = BUS_get_branch_k(bus); br != NULL; br = BRANCH_get_next_k(br)) {
        ...
        // I^2 R = + a_km^2*vk^2*g + vm^2*g - a_km*vm*vk*y(cos(wk-wm-phsh-theta_brn) + cos(wm-wk+phsh-theta_brn))
        // 
        (*phi) += pow(a_km,2)*pow(vk,2)*g + pow(vm,2)*g - a_km*vk*vm*y*(cos_w_k_m_ph_th + cos_w_m_k_ph_th);
        ...
        // vk
        if (BUS_has_flags(bus, FLAG_VARS, BUS_VAR_VMAG)) {
            gphi[index_vk] += 2 * pow(a_km, 2) * vk * g - a_km * vm * y * (cos_w_k_m_ph_th + cos_w_m_k_ph_th);
            hphi_vk += 2 * pow(a_km, 2) * g;  // vk&vk

            if (BUS_has_flags(bus, FLAG_VARS, BUS_VAR_VANG)) { // vk wk
                hphi_vkwk += -a_km * vm * y * (-sin_w_k_m_ph_th + sin_w_m_k_ph_th);
            }

            if (BUS_has_flags(bus_m, FLAG_VARS, BUS_VAR_VMAG)) { // vk vm
                Hphi[*Hphi_nnz] += -a_km * y * (cos_w_k_m_ph_th + cos_w_m_k_ph_th);
                (*Hphi_nnz)++;
            }

            if (BUS_has_flags(bus_m, FLAG_VARS, BUS_VAR_VANG)) { // vk wm
                Hphi[*Hphi_nnz] += -a_km * vm * y * (sin_w_k_m_ph_th - sin_w_m_k_ph_th);
                (*Hphi_nnz)++;
            }

            if (BRANCH_has_flags(br, FLAG_VARS, BRANCH_VAR_RATIO)) { // vk a_km
                Hphi[*Hphi_nnz] += 4 * a_km * vk * g - vm * y * (cos_w_k_m_ph_th + cos_w_m_k_ph_th);
                (*Hphi_nnz)++;
            }
        ...
        // wk
        if (BUS_has_flags(bus, FLAG_VARS, BUS_VAR_VANG)) {
            gphi[index_wk] += -a_km * vk * vm * y * (-sin_w_k_m_ph_th + sin_w_m_k_ph_th);
            hphi_wk += -a_km * vk * vm * y * (-cos_w_k_m_ph_th - cos_w_m_k_ph_th);  // wk&wk

            if (BUS_has_flags(bus_m, FLAG_VARS, BUS_VAR_VMAG)) { // wk vm
                Hphi[*Hphi_nnz] += -a_km * vk * y * (-sin_w_k_m_ph_th + sin_w_m_k_ph_th);
                (*Hphi_nnz)++;
            }

            if (BUS_has_flags(bus_m, FLAG_VARS, BUS_VAR_VANG)) { // wk wm
                Hphi[*Hphi_nnz] += -a_km * vk * vm * y * (cos_w_k_m_ph_th + cos_w_m_k_ph_th);
                (*Hphi_nnz)++;
            }

            if (BRANCH_has_flags(br, FLAG_VARS, BRANCH_VAR_RATIO)) { // wk a_km
                Hphi[*Hphi_nnz] += -vk * vm * y * (-sin_w_k_m_ph_th + sin_w_m_k_ph_th);
                (*Hphi_nnz)++;
            }
        ...
        // vm
        if (BUS_has_flags(bus_m, FLAG_VARS, BUS_VAR_VMAG)) {
            gphi[index_vm] += 2 * vm * g - a_km * vk * y * (cos_w_k_m_ph_th + cos_w_m_k_ph_th);
            Hphi[*Hphi_nnz] += 2 * g;  // vm&vm
            (*Hphi_nnz)++;
            ...
            if (BRANCH_has_flags(br, FLAG_VARS, BRANCH_VAR_RATIO)) { // vm a_km
                Hphi[*Hphi_nnz] += -vk * y * (cos_w_k_m_ph_th + cos_w_m_k_ph_th);
                (*Hphi_nnz)++;
            }
        ...
        // wm
        if (BUS_has_flags(bus_m, FLAG_VARS, BUS_VAR_VANG)) {
            gphi[index_wm] += -a_km * vk * vm * y * (sin_w_k_m_ph_th - sin_w_m_k_ph_th);
            Hphi[*Hphi_nnz] += -a_km * vk * vm * y * (-cos_w_k_m_ph_th - cos_w_m_k_ph_th);  // wm&wm
            (*Hphi_nnz)++;
        ...
        // a_km 
        if (BRANCH_has_flags(br, FLAG_VARS, BRANCH_VAR_RATIO)) {
            gphi[index_a_km] += 2 * a_km * pow(vk, 2.) * g - vk * vm * y * (cos_w_k_m_ph_th + cos_w_m_k_ph_th);
            Hphi[*Hphi_nnz] += 2 * pow(vk, 2.) * g;  // a_km a_km
            (*Hphi_nnz)++;
        ...
        // phsh
        if (BRANCH_has_flags(br, FLAG_VARS, BRANCH_VAR_PHASE)) {
            gphi[index_phsh] += -a_km * vk * vm * y * (sin_w_k_m_ph_th - sin_w_m_k_ph_th);
            Hphi[*Hphi_nnz] += -a_km * vk * vm * y * (-cos_w_k_m_ph_th - cos_w_m_k_ph_th);   // phsh phsh
            (*Hphi_nnz)++;
        }
    }
